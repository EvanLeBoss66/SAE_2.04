...
#sauvegarde
def sv(df, nom): df.to_csv(nom, index=False, sep=';'); print(nom, len(df))
...
regs = [{'ID_ORGANISME':o, 'ID_REGISTRE':1, 'DATE_OUVERTURE':fake.date_between('-5y', '-1y'), 'DERNIERE_MODIFICAITON':fake.date_between('-1y', 'today')} for o in org_ids]
...
for deb in range(0, total, lot):
    ...
    paires.update(zip(orgs[:1500], ids[:1500]))
    ...
    pd.DataFrame({
        ...
        'ID_GENRE': [random.randint(1,3) for _ in range(nb)], 'ID_TITRE': [random.randint(1,3) for _ in range(nb)],
        'ID_RANG': [random.randint(1,3) for _ in range(nb)], 'ID_DIGNITE': [random.randint(1,3) for _ in range(nb)],
        'ID_GRADE': [random.randint(1,5) for _ in range(nb)]
    }).to_csv(fcsv, mode='a', index=False, header=(deb==0), sep=';')

ech = list(paires)

#cartes
ec, timp = ech[:min(10000, len(ech))], ech[:min(5000, len(ech))]
...
#certifs
certs = [{'ID_CERTIFICATION': f'CERT_{i}', 'DATE_DE_CERTIFICATION': str(fake.date_between('-1y', 'today')), 'ID_ORGANISME': random.choice(timp)[0], 'ID_MEMBRE': random.choice(timp)[1]} for i in range(1, nb_mac+1)]
...
g('reunion.csv', 5000, lambda: (*random.choice(ech), random.randint(1, 10000)), ['ID_ORGANISME','ID_MEMBRE','ID_RDV'])
...
g('donne.csv', 1000, lambda: (random.choice(ing_ids[:300]), random.randint(1, 50)), ['ID_INGREDIENT','ID_ALLERGENE'])
...
g('heurt.csv', 1000, lambda: (random.choice(ing_ids[:300]), random.randint(1, 20)), ['ID_INGREDIENT','ID_CROYANCE'])
...
g('transgresse.csv', 1000, lambda: (random.choice(ing_ids[:300]), random.randint(1, 20)), ['ID_INGREDIENT','ID_CONVICTION'])
