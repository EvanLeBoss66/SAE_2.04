...
vraies_sauces = [
    ...
    "Sauce au poivre", "Sauce hollandaise", "Sauce tomate", "Vinaigrette", 
    "Sauce tartare", "Sauce pesto", "Sauce soja", "Sauce barbecue", "Sauce moutarde"
]

vrais_ingredients = [
    "Sel", "Poivre", "Huile d'olive", "Farine", "Sucre", "Beurre", "Ail", 
    "Oignon", "Tomate", "Poulet", "Boeuf", "Crème fraîche", "Basilic", 
    "Persil", "Pâtes", "Riz", "Oeuf", "Lardon", "Champignon", "Fromage",
    "Carotte", "Poireau", "Courgette", "Pomme de terre", "Thon", "Saumon"
]

vrais_plats = [
    "Blanquette de veau", "Boeuf bourguignon", "Gratin dauphinois", 
    "Quiche lorraine", "Pâtes à la carbonara", "Poulet rôti", 
    "Lasagnes", "Pizza Margherita", "Ratatouille", "Hachis parmentier",
    "Couscous", "Choucroute", "Cassoulet", "Burger maison", "Salade césar"
]

vraies_croyances = [
    "Halal", "Casher", "Végétarien", "Végétalien", "Carême", "Ramadan"
]

vraies_convictions = [
    "Zéro déchet", "Bio", "Local / Circuit court", "Éco-responsable", "Sans huile de palme"
]

vrais_allergenes = [
    "Arachides", "Lait", "Œufs", "Gluten", "Poisson", 
    "Crustacés", "Soja", "Fruits à coque", "Moutarde", "Céleri"
]

def sv(df, nom): 
    df.to_csv(nom, index=False, sep=';')
    print(f"Généré : {nom} ({len(df)} lignes)")
...
sv(pd.DataFrame({'ID_ALLERGENE': range(1, len(vrais_allergenes)+1), 'NOM': vrais_allergenes}), 'allergies.csv')
sv(pd.DataFrame({'ID_CROYANCE': range(1, len(vraies_croyances)+1), 'NOM': vraies_croyances}), 'croyance.csv')
sv(pd.DataFrame({'ID_CONVICTION': range(1, len(vraies_convictions)+1), 'CONVICTION': vraies_convictions}), 'convictions.csv')
...
sv(pd.DataFrame({'ID_ORGANISME': [x[0] for x in timp], 'ID_MEMBRE': [x[1] for x in timp], 'POSITION_': [fake.job()[:50] for _ in timp]}), 'tenrac_important.csv')
...
tent = [{'ID_CERTIFICATION': c['ID_CERTIFICATION'], 'ID_TYPE': tid, 'TYPE': f'Type {tid}', 'NOM': f"Entretien {fake.word()[:40]}", 'PERIODICITE': 'Mensuelle'} for c in certs for tid in (1, 2)]
...
sv(pd.DataFrame([{'ID_MACHINE': m['ID_MACHINE'], 'ID_MODELE': 1, 'NOM': f"Modèle {fake.word()[:40]}", 'MARQUE': fake.company()[:50], 'ID_CERTIFICATION': m['ID_CERTIFICATION'], 'ID_TYPE': random.choice([t['ID_TYPE'] for t in tent if t['ID_CERTIFICATION'] == m['ID_CERTIFICATION']])} for m in machs]), 'modele.csv')
...
max_allergene_id = len(vrais_allergenes)
max_croyance_id = len(vraies_croyances)
max_conviction_id = len(vraies_convictions)
...
g('donne.csv', 1000, lambda: (random.choice(ing_ids), random.randint(1, max_allergene_id)), ['ID_INGREDIENT','ID_ALLERGENE'])
g('accompagne.csv', 1000, lambda: (random.choice(plt_ids), random.choice(sau_ids)), ['ID_PLAT','ID_SAUCE'])
g('allergique_a_.csv', 5000, lambda: (*random.choice(ech), random.randint(1, max_allergene_id)), ['ID_ORGANISME','ID_MEMBRE','ID_ALLERGENE'])
g('heurt.csv', 1000, lambda: (random.choice(ing_ids), random.randint(1, max_croyance_id)), ['ID_INGREDIENT','ID_CROYANCE'])
g('croit.csv', 5000, lambda: (*random.choice(ech), random.randint(1, max_croyance_id)), ['ID_ORGANISME','ID_MEMBRE','ID_CROYANCE'])
g('a_des.csv', 5000, lambda: (*random.choice(ech), random.randint(1, max_conviction_id)), ['ID_ORGANISME','ID_MEMBRE','ID_CONVICTION'])
g('transgresse.csv', 1000, lambda: (random.choice(ing_ids), random.randint(1, max_conviction_id)), ['ID_INGREDIENT','ID_CONVICTION'])
